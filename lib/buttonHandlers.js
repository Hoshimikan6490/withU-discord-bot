...
async function handleUniversityNameInputModal(interaction, buttonId) {
	const customId = buttonId === "reShowUniversityNameInputModal" 
		? "reAskUniversityName" 
		: "askUniversityName";
...
async function handleOrganizationNameInputModal(interaction, buttonId) {
	const customId = buttonId === "reShowOrganizationNameInputModal"
...
async function handleOrganizationNameConfirm(interaction) {
	const messageId = interaction.message.embeds[0].footer.text.split("管理ID: ")[1];
	...
	await organizationRegister(interaction.client, interaction, userId, messageId);
}
...
async function handleButtonInteraction(client, interaction) {
	...
	try {
		...
		} else if (buttonId.toLowerCase().includes("showUniversityNameInputModal".toLowerCase())) {
			await handleUniversityNameInputModal(interaction, buttonId);
		} else if (buttonId.toLowerCase().includes("showOrganizationNameInputModal".toLowerCase())) {
