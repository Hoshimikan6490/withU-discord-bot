...
class ErrorHandler {
	 ...
	 */
	static async handle(error, interaction = null, userMessage = ERROR_MESSAGES.GENERIC_ERROR, ephemeral = true) {
		// Log to Sentry
	...
	static async handleSelfIntroductionError(error, interaction) {
		await this.handle(error, interaction, ERROR_MESSAGES.SELF_INTRODUCTION_ERROR);
	}
