...
const { EMBED_COLORS, EMBED_TITLES } = require('../config/constants');
...
async function sendJoinProcessLog(client, type, howToSet, userId) {
	try {
		...
		const { member } = await fetchGuildAndMember(client, userId);
...
function buildSelfIntroductionDescription(fields, modalId) {
	if (modalId === 'userSelfIntroductionModal') {
		...
		const organizationDescription = fields.getTextInputValue(
 ...
 */
async function postSelfIntroduction(
	client,
	userName,
	description,
	userId,
	...
	await client.channels.cache.get(process.env.selfIntroductionChannelID).send({
		content: `<@${userId}>`,
		embeds: [embed],
	});
}
