...
const { getDatabaseFromSchoolName } = require("./databaseController");
const { 
	createStandardEmbed,
...
async function handleUniversityNameModal(interaction, modalId) {
	const universityNameInput = interaction.fields.getTextInputValue("universityNameInput");
	...
	if (modalId === "reAskUniversityName") {
		...
		await disablePreviousMessage(interaction, "下のボタンから大学名/組織名を設定してください");
	}
}
...
async function handleOrganizationNameModal(interaction, modalId) {
	const organizationNameInput = interaction.fields.getTextInputValue("organizationNameInput");
...
async function handleSelfIntroductionModal(interaction, modalId) {
	...
	try {
		...
		const description = buildSelfIntroductionDescription(interaction.fields, modalId);
...
async function handleModalInteraction(client, interaction) {
	...
	try {
		...
		} else if (modalId.toLowerCase().includes("askOrganizationName".toLowerCase())) {
